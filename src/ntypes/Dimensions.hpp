#ifndef _NUPIC_NTYPES_DIMENSIONS_H_
#define _NUPIC_NTYPES_DIMENSIONS_H_
...
export interface Dimensions_Static {
	...
	//  */
	// new( x: number,  y: number): Dimensions;
...
export interface Dimensions extends Array<number> { // : public std::vector < size_t >
	 ...
	 */
	getIndex(coordinate: Coordinate): number;
	 ...
	 */
	toString(humanReadable?: boolean /*= true*/): string;
	 ...
	 */
	promote(newDimensionality: number): void;
...
export let Dimensions: Dimensions_Static = nupic_module.Dimensions;
#endif