#ifndef _NUPIC_ENGINE_LINK_H_
#define _NUPIC_ENGINE_LINK_H_
...
export interface Link_Static {
	...
	new(
		...
		srcRegionName: string, destRegionName: string,
		srcOutputName?: string /*=""*/,
		destInputName?: string /*=""*/,
		propagationDelay?: number/*=0*/): Link;
	...
	new(
		linkType: string, linkParams: string,
		srcOutput: Output, destInput: Input, propagationDelay?: number /*=0*/): Link;
...
export interface Link {// : public Serializable<LinkProto>
	// {
	// public:
     ...
     *            The destination Input of the link
     */
	connectToNetwork(src: Output, dest: Input): void;
     ...
     *
     */
	initialize(destinationOffset: number): void;
...
// #endif // NTA_LINK_HPP
export let Link: Link_Static = nupic_module.Link;
#endif